...
async function getParams() {
  ...
  const getPreviousMonthGames = (month) => {
    return pastGames.filter((game) => {
      const gameDate = new Date(game.date);
      return (
        gameDate.getMonth() === month && gameDate.getFullYear() === currentYear
      );
    });
  };

  let currentMonthGames = getPreviousMonthGames(currentMonth);
  ...
  let previousMonthGames = getPreviousMonthGames(previousMonth);

  while (previousMonthGames.length === 0 && previousMonth > 0) {
    previousMonth--;
    previousMonthGames = getPreviousMonthGames(previousMonth);
  }
  ...
  let losingMonths = 0;
  let checkingMonth = previousMonth;

  while (checkingMonth >= 0 && loserId != null) {
    let checkingMonthGames = getPreviousMonthGames(checkingMonth);

    while (checkingMonthGames.length === 0 && checkingMonth > 0) {
      checkingMonth--;
      checkingMonthGames = getPreviousMonthGames(checkingMonth);
    }

    if (checkingMonthGames.length === 0) {
      break;
    }
