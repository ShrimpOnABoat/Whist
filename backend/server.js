...
async function saveScoreInDB() {
  try {
    ...
    const gameData = {
      ...
      gg_score: gameState.players
        .find((player) => player.username === "gg")
        .scores.slice(-1)[0],
      gg_position: getScorePosition("gg"),
      dd_score: gameState.players
        .find((player) => player.username === "dd")
        .scores.slice(-1)[0],
      dd_position: getScorePosition("dd"),
      toto_score: gameState.players
        .find((player) => player.username === "toto")
        .scores.slice(-1)[0],
      toto_position: getScorePosition("toto"),
    };
...
router.get("/scores/:year", authenticate, async (req, res) => {
  try {
    ...
    if (fs.existsSync(filePath)) {
      ...
      res
        .status(404)
        .json({ message: `No scores found for the year ${year}.` });
    }
  } catch (error) {
    // Handle any errors
    console.error(error);
...
function lastPlayerNotDiscarding() {
  ...
  const lastPositionPlayer = gameState.players.find(
    (player) => getPosition(player.username) === 3
  );
    ...
    if (gameState.round < 4) {
      if (
        !gameState.players.every(
          (everyPlayer) =>
            everyPlayer.announcedTricks.length > gameState.round - 1
        )
      ) {
      ...
      const anyPlayerPlaying = gameState.players.some(
        (player) =>
          player.action === "playCard" || player.action === "grabTrick"
      );
      if (
        getPosition(username) === 1 &&
        player.monthlyLosses < 3 &&
        !allScoresEqual() &&
        !anyPlayerPlaying &&
        lastPlayerNotDiscarding()
      ) {
      ...
      if (
        !gameState.players.every(
          (everyPlayer) =>
            everyPlayer.announcedTricks.length === gameState.round
        )
      ) {
        if (
          player.username !== username &&
          player.announcedTricks.length === gameState.round
        ) {
...
function allScoresEqual() {
  ...
  if (players.length > 0) {
    ...
    return players.every(
      (player) => player.scores[player.scores.length - 1] === lastScore
    );
  }
  return true;
}
...
function dealCards() {
  ...
  for (let i = 0; i < gameState.players.length; i++) {
    ...
    if (gameState.round > 3) {
      if (playerPosition === 2) {
        ...
        const secondPlayerScore =
          gameState.players[1].scores[gameState.round - 2];
        if (
          gameState.players[i].scores[gameState.round - 2] <=
          0.5 * secondPlayerScore
        ) {
    ...
    if (
      ...
      extraCards === 2 &&
      gameState.players[1].scores[gameState.round - 2] !==
        gameState.players[2].scores[gameState.round - 2]
    ) {
      extraCards = 1;
    }
    for (let j = 0; j < cardsToDeal + extraCards; j++) {
      if (gameState.deck.length > 0) {
        ...
        let lastCard =
          gameState.players[i].hand[gameState.players[i].hand.length - 1];
...
function sendUpdate() {
  ...
  io.to(usernameToSocketId["gg"]).emit(
    "updateGameState",
    createPublicGameState("gg")
  );
  io.to(usernameToSocketId["dd"]).emit(
    "updateGameState",
    createPublicGameState("dd")
  );
  io.to(usernameToSocketId["toto"]).emit(
    "updateGameState",
    createPublicGameState("toto")
  );
}
...
function getPosition(username) {
  ...
  const highestScore = Math.max(
    ...gameState.players.map((p) => p.scores[p.scores.length - 1])
  );
  ...
  if (player.scores[player.scores.length - 1] === lowestScore) {
    ...
    const sortedScores = [...gameState.players].sort(
      (a, b) => a.scores[a.scores.length - 1] - b.scores[b.scores.length - 1]
    );
    const secondLowest =
      sortedScores[1].scores[sortedScores[1].scores.length - 1];
    ...
    if (player.scores[player.scores.length - 1] === secondLowest) {
      ...
      while (previousRound >= 0) {
        const previousRoundScores = playersWithLowestScore.map(
          (p) => p.scores[previousRound]
        );
        const lowestPreviousRoundScore = Math.min(...previousRoundScores);

        if (lowestPreviousRoundScore !== secondLowest) {
          const indexOfPlayerWithLowestScore = previousRoundScores.indexOf(
            lowestPreviousRoundScore
          );
          if (indexOfPlayerWithLowestScore > -1) {
            // Add this check
            if (
              playersWithLowestScore[indexOfPlayerWithLowestScore].username ===
              username
            ) {
              return 3;
            } else {
      ...
      const startingPlayerIndex = gameState.playOrder.indexOf(
        gameState.startingPlayer
      );
      const nextPlayerIndex =
        (startingPlayerIndex + 1) % gameState.playOrder.length;
      if (
        nextPlayerIndex === sortedScores[1].playerId ||
        nextPlayerIndex === sortedScores[2].playerId
      ) {
        return 3;
      } else {
        return 2;
...
function getScorePosition(playerUsername) {
  // Initial sorting by the latest score to establish a preliminary ranking
  const sortedByCurrentScore = [...gameState.players].sort(
    (a, b) => b.scores.slice(-1)[0] - a.scores.slice(-1)[0]
  );

  // Recursive function to apply tie-breaking based on historical scores
  function applyTieBreaking(sortedPlayers) {
    for (let i = 0; i < sortedPlayers.length; i++) {
      // Detect tie
      if (
        i + 1 < sortedPlayers.length &&
        sortedPlayers[i].scores.slice(-1)[0] ===
          sortedPlayers[i + 1].scores.slice(-1)[0]
      ) {
        let tieEnd = i + 1;
        // Find the range of players with tied scores
        while (
          tieEnd < sortedPlayers.length &&
          sortedPlayers[i].scores.slice(-1)[0] ===
            sortedPlayers[tieEnd].scores.slice(-1)[0]
        ) {
          tieEnd++;
        }

        // Process the tie by looking at historical scores, starting from the most recent
        for (
          let round = sortedPlayers[i].scores.length - 2;
          round >= 0;
          round--
        ) {
          const tieGroup = sortedPlayers.slice(i, tieEnd);
          tieGroup.sort((a, b) => b.scores[round] - a.scores[round]);

          // Check if the tie is resolved at this round
          if (
            !tieGroup.some(
              (player, index) =>
                index > 0 &&
                player.scores[round] === tieGroup[index - 1].scores[round]
            )
          ) {
            sortedPlayers.splice(i, tieEnd - i, ...tieGroup); // Replace the tied group with the resolved order
            break; // Exit the loop once the tie is resolved
          }
        }

        i = tieEnd - 1; // Adjust the index to skip the resolved tie group
      }
    }
    return sortedPlayers;
  }

  const finalRanking = applyTieBreaking(sortedByCurrentScore);

  // Determine the position of the requested player in the final ranking
  return (
    finalRanking.findIndex((player) => player.username === playerUsername) + 1
...
function newRound() {
  ...
  gameState.startingPlayer =
    gameState.playOrder[gameState.round % gameState.playOrder.length];
  ...
  for (let i = 0; i < gameState.players.length; i++) {
    ...
    if (gameState.round < 4) {
      ...
      console.log(
        gameState.players[i].username + " est en position: " + position
      );
      switch (position) {
        case 1:
          gameState.players[i].action = "bet";
...
function determineTrickWinner() {
  ...
  const trumpCards = gameState.trickCards.filter(
    (card) => card && card.suit === gameState.trumpCard.suit
  );

  let winner;
  if (trumpCards.length > 0) {
    // if there are trump cards, the highest trump wins
    const highestTrump = trumpCards.sort(
      (a, b) => rankValue[b.rank] - rankValue[a.rank]
    )[0];
    const winnerCardIndex = gameState.trickCards.indexOf(highestTrump);
    winner = gameState.players.find(
      (player) => player.username === gameState.playOrder[winnerCardIndex]
    );
  } else {
    // if there are no trumps, the highest card of the leading suit wins
    const leadingSuitCards = gameState.trickCards.filter(
      (card) => card && card.suit === leadingSuit
    );
    const highestLeadingSuitCard = leadingSuitCards.sort(
      (a, b) => rankValue[b.rank] - rankValue[a.rank]
    )[0];
    const winnerCardIndex = gameState.trickCards.indexOf(
      highestLeadingSuitCard
    );
    winner = gameState.players.find(
      (player) => player.username === gameState.playOrder[winnerCardIndex]
    );
...
function calculateAndStoreScore() {
  ...
  const maxTotalAnnouncedTricks = Math.max(
    ...gameState.players.map((player) =>
      player.announcedTricks.reduce((a, b) => a + b, 0)
    )
  );
  ...
  gameState.players.forEach((player) => {
    ...
    if (
      player.madeTricks[player.madeTricks.length - 1] ===
      player.announcedTricks[player.announcedTricks.length - 1]
    ) {
      score = 10; // Base score
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
    console.dir(blueColor + "socket.on(disconnect)" + resetColor, {
      depth: null,
    });
...
*/
  ...
  socket.on("discard", (cards) => {
    ...
    if (player.action === "discard") {
      ...
      player.hand = player.hand.filter(
        (handCard) =>
          !cards.some(
            (card) => card.rank === handCard.rank && card.suit === handCard.suit
          )
      );

      // Get the maximum value of bonusCards from each player
      const maxBonusCards = gameState.players
        .map((player) => player.bonusCards)
      ...
      if (
        ...
        console.log(
          player.username + " donne sa carte à " + lastPlayer.username
        );
      ...
      if (playerRank === 2) {
        ...
        playerRank === 3 &&
        otherPlayers.every((p) => p.action === "waitForPlayer")
      ) {
        player.action = "waitForPlayer";
        gameState.players.find(
          (p) => p.username === gameState.startingPlayer
        ).action = "playCard";
      } else {
        player.action = "waitForPlayer";
      }
  ...
  function isScoreTwiceAsHigh(playerIndex) {
    const playerScore =
      gameState.players[playerIndex].scores[gameState.round - 2];
    ...
    for (let i = 0; i < otherPlayerScores.length; i++) {
      if (
        typeof otherPlayerScores[i] !== "undefined" &&
        playerScore < 2 * otherPlayerScores[i]
      ) {
        return false;
      }
    }
  ...
  socket.on("placeBet", (betValue) => {
    ...
    const allAnnounced = gameState.players.every(
      (player) => player.announcedTricks.length === gameState.round
    );
    ...
    if (gameState.round < 4) {
      if (
        gameState.players.every(
          (p) => p.announcedTricks.length === gameState.round
        )
      ) {
        ...
        gameState.players.find(
          (p) => p.username === gameState.playOrder[gameState.round - 1]
        ).action = "playCard";
      } else {
        // Get the next player according to the play order
        let nextPlayerUsername =
          gameState.playOrder[
            (currentPlayerIndex + 1) % gameState.playOrder.length
          ];
        const nextPlayer = gameState.players.find(
          (p) => p.username === nextPlayerUsername
        );

        // Set the action for the next player who needs to place a bet and for others to 'waitForPlayer'
        gameState.players.forEach(
          (p) => (p.action = p === nextPlayer ? "bet" : "waitForPlayer")
        );
      }
    } else {
      ...
      if (!allScoresEqual() && allAnnounced) {
        ...
        const isScoreTwiceAsHigh = otherPlayerScores.every(
          (score) => playerScore >= 2 * score
        );
  ...
  socket.on("playCard", (card) => {
    ...
    if (
      gameState.trickCards.filter(Boolean).length === gameState.players.length
    ) {
      ...
      gameState.players.forEach((p) => {
        console.log(winner);
        p.action =
          p.username === winner.username ? "grabTrick" : "waitForPlayer";
      });
    } else {
      // otherwise, the next player in the playOrder gets 'playCard' and the others should be at 'waitForPlayer'
      const nextPlayerIndex =
        (playOrderPosition + 1) % gameState.players.length; // gets the index of the next player
      const nextPlayer = gameState.players.find(
        (player) => player.username === gameState.playOrder[nextPlayerIndex]
      );
      gameState.players.forEach((p) => {
        p.action =
          p.username === nextPlayer.username ? "playCard" : "waitForPlayer";
      });
    }
  ...
  socket.on("grabTrick", () => {
    ...
    console.dir(
      blueColor + "socket.on(grabTrick) by " + player.username + resetColor,
      { depth: null }
    );
    const otherPlayers = gameState.players.filter(
      (p) => p.playerId !== socket.id
    ...
    console.log(
      player.username +
        " now has " +
        player.madeTricks[gameState.round - 1] +
        " tricks"
    );
      ...
      if (!isLastRound) {
        ...
        const highestScore = Math.max(
          ...gameState.players.map(
            (player) => player.scores[gameState.round - 1]
          )
        );

        // Identify all players with the highest score
        const winners = gameState.players.filter(
          (player) => player.scores[gameState.round - 1] === highestScore
        );
